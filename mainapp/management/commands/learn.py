...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        # home_cat_query = Q(category__name='дом')
        # office_cat_query = Q(category__name='фис')
        #
        # product_list = Product.objects.filter(home_cat_query | office_cat_query)
        #
        # print(product_list)
        ...
        for item in orderitem_items:
            print(f'{item.action_order:2} order: #{item.pk:3} discount: {item.total_price} '
                  f'product: {item.product.name} {item.order.updated_at - item.order.created_at}')
